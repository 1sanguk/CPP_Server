...
class EpollEchoServer{
    private:
        ...
        // v4는 단일 이벤트 루프지만, main thread의 Stop()과 server thread의 상태 공유가 있다.
        // stopping, cleaned_up, client_fds 접근은 queue_mutex로 보호한다.
        std::mutex queue_mutex;
        // 현재 epoll에 등록되어 있고 아직 닫히지 않은 client fd 목록.
        std::unordered_set<int> client_fds{};
        
        // Stop()이 true로 바꾸며, event loop의 while 조건과 종료 분기에서 확인한다.
        bool stopping = false;
        // Server_Run() 종료 경로와 소멸자 경로에서 cleanup이 중복 실행되는 것을 막는다.
    ...
    public:
        ...
        // main thread에서 호출하는 종료 요청 함수. eventfd에 write해 epoll_wait()를 깨운다.
        void Stop();
